        ...
        try {
            ...
            if (prIds.length > 0) {
                ...
                if (data.length > 1) {
                    ...
                    if (state === "closed" && res.user.login === res.closed_by.login) status.push(`<t:${Math.floor(new Date(res.closed_at).getTime() / 1000)}:R>`);
                    if (state === "closed" && res.user.login !== res.closed_by.login) status.push(`<t:${Math.floor(new Date(res.closed_at).getTime() / 1000)}:R> by [${res.closed_by.login}](${res.closed_by.html_url})`);
